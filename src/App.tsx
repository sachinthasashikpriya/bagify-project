...
import { mockProducts, mockSellers, mockBuyers, mockOrders } from './data/mockData';
import { User, Product, CartItem, Seller, Review } from './types';

type Page = 'home' | 'product' | 'seller-dashboard' | 'buyer-dashboard' | 'admin-dashboard' | 'login' | 'cart';
...
function App() {
  ...
  const handleCheckout = () => {
    ...
    const newOrder = {
      ...
      totalAmount: cartItems.reduce((sum, item) => sum + (item.product.price * item.quantity), 0) + 10,
  ...
  const handleAddProduct = (productData: Omit<Product, 'id' | 'sellerId' | 'sellerName' | 'sellerRating' | 'reviews' | 'averageRating'>) => {
  ...
  const currentUserOrders = currentUser ? orders.filter(o => o.buyerId === currentUser.id) : [];
